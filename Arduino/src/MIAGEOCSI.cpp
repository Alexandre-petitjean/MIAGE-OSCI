...
void sendRc522Data(String thingName){
...
boolean connectToWiFi(int timeout, String epromSSID, String epromPWD)
{
  ...
  { // check that WiFi is connected, print status and device IP address before returning
  ...
  { // if WiFi is not connected we must have exceeded WiFi connection timeout
    return false;
  }
}
...
void ReloadConfiguration()
{
  ...
  SENSOR_RC522 = thingworx.httpGetBoolPropertry(thingname, "SENSOR_RC522");
...
{                                  // Page d'accueil La page HTML est mise dans le String page
  String page = "<!DOCTYPE html>"; // Début page HTML
  page += "<head>";
  ...
  page += "    <p>SENSOR_RC522    :" + String(SENSOR_RC522) + "</p>";
...
void loop()
{
  ...
  { // confirm WiFi is connected before looping as long as WiFi is connected
    // Managing WebClient :
    // server.handleClient();
    manageBase(thingName);
    // Acquire Values from the sensors depending on configuration
    if (SENSOR_MAG)
      sendMagData(thingName);
    if (SENSOR_LCD)
      displayValues();
    if (SENSOR_LIGHT)
      sendLightData(thingName);
    if (SENSOR_BARO)
      sendBaroData(thingName);
    if (SENSOR_ACCEL)
      sendAccelData(thingName);
    if (SENSOR_SMB380)
      sendSMB380Data(thingName);
    if (SENSOR_TEMP)
      sendTempData(thingName);
    if (SENSOR_GPS)
      sendGPSData(thingName);
    if (SENSOR_WEATHER)
      sendWeatherData(thingName);
    
    if (SENSOR_RC522)
      sendRc522Data(thingName)
    i++;
